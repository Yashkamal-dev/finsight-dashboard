...
const Navbar = () => {
  ...
  return (
    <aside
      className={`fixed bottom-0 left-0 z-10 grid w-full gap-4 overflow-auto bg-[var(--bg-secondary)] p-4 md:sticky md:top-0 md:z-0 md:h-screen md:auto-rows-max ${collapsed ? "md:w-25" : "md:w-72"} transition-all duration-300 ease-in-out`}
    >
      {/* product name and logo */}
      <header
        className={`hidden items-center gap-3 py-2 md:flex ${collapsed ? "justify-center" : "px-2"}`}
      >
      ...
      <div className="flex items-center justify-between md:block">
              ...
              {({ isActive }) => {
                return (
                  <>
                    ...
                    <span
                      className={`hidden w-full text-xl text-[var(--text-primary)] md:block ${collapsed ? "md:hidden" : ""} ${isActive ? "text-white" : ""} `}
                    >
                      Dashboard
                    </span>
                  </>
        ...
        {/* theme toggle button */}
        <div
          className={`md:absolute md:bottom-10 md:left-0 ${collapsed === false && "pl-6"} flex justify-center md:w-full ${collapsed === false ? "justify-start" : ""} `}
        >
          <button
            onClick={() => {
              setIsDark(!isDark);
            }}
            className={`cursor-pointer rounded-full bg-[var(--accent-primary)] p-2.5 md:p-3`}
          >
            {isDark === true ? (
